...
- 差出人用郵便番号枠（はがき左下）は同マニュアルに記載が無く、根拠となる公式資料が
  見つからなかった（2026-06-25追加）。代わりに、ユーザー提供の実際のラクスル発注
  ドラフトPDF（`past_operation/2025宛名面.pdf`）にガイド線として実在する枠の座標を
  実測して採用した。ラクスルの印刷代行サービス用テンプレートの値であり、本アプリの
  自宅プリンタ直接印刷とは出力経路が異なる点に注意（実際の自宅プリンタでの印刷結果
  との一致は別途要実機確認）。
- 印刷可能領域の安全マージンはCanon GX7130相当機種(GX7030公式FAQ)の値。
  なお、GX7130ははがきサイズの真のフチなし印刷に対応しておらず、印刷可能領域に
  収めるため出力全体を自動的に縮小・再配置してしまうことを2026-07-03に実機で確認した。
  この補正はレイアウト計算(本ファイル)には含めず、生成後のPDFに対して独立に適用する
  アダプタ（`printer_adjust.py`、README.md「機能: プリンタ調整」参照）で行う。
- 郵便番号枠は受取人・差出人とも、現物では3桁目と4桁目の間に住所の「123-4567」の
  ...
  （ユーザー指摘により発覚）。差出人側は元々ラクスル発注ドラフトPDFの実測で約1mmと
  分かっていたが、従来は簡略化のため未実装だった。両者ともここで初めて反映し、値は
  0.7mmを暫定の共通既定値として使う（実機での見え方次第で今後調整可能。
  README.md「宛名印字 PDF」参照）。
"""
import os
import subprocess
import tempfile
...
ZIP_BOX_DEFAULTS = {
    ...
    'pitch_mm': 7.0,
    'font_pt': 15,
    # 3桁目と4桁目の間だけに入る余分な隙間（2026-07-03実測の暫定値、モジュール冒頭の
    # docstring参照）。
    'hyphen_gap_mm': 0.7,
}
...
# 各値はモジュール冒頭のdocstringに記載の各項目と同様、プレビューUIでの調整可能
# パラメータとして扱う想定（既定値）。
